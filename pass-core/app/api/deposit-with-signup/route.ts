...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await withTimeout(
      handleDeposit(req, (k, v) => { if (k === "userId") createdUserId = v; if (k === "merchantId") createdMerchantId = v; if (k === "sessionToken") sessionToken = v; }),
      REQUEST_DEADLINE_MS,
      "deposit-with-signup"
    );
...
async function handleDeposit(req: NextRequest, notify: StateNotify): Promise<NextResponse> {
  ...
  if (!user) {
    ...
    // timeout postgres si le pooler est dégradé. Cause directe identifiée du
    // hang en production le 29 avril 2026.
  ...
  if (createdUserId) {
    ...
    notify("sessionToken", sessionTokenLocal);
  }
  ...
  const response = NextResponse.json({
    ...
    warnings,
    created: { user: !!createdUserId, merchant: !!createdMerchantId, artwork: true },
  });
