...
import {
  ...
  Fingerprint, Eye, Mic, MicOff, AlertTriangle, Mail,
} from "lucide-react";
...
// These flags reflect which backend services are configured.
// When a service is down, the UI shows a clear notice instead
// of failing silently or crashing.
// ═══════════════════════════════════════════════════════════
const SERVICES = {
  // Stripe webhook + platform account — needed for commissions & paid plans
  stripe: !!(
    process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY &&
    !process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY.includes("REMPLACE")
  ),
  // Email notifications via Resend
  email: false, // overridden at runtime via /api/certify response
};
...
export default function CertifierPage() {
  ...
  function toggleListening() {
    ...
    if (!SpeechRecognition) {
      alert("La reconnaissance vocale n'est pas supportee par ce navigateur. Utilisez Chrome ou Safari.");
      return;
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!res.ok) {
        // Graceful error messages for known missing-config cases
        if (data.error?.includes("stripe") || data.error?.includes("Stripe") || data.error?.includes("payment")) {
          throw new Error("Paiements temporairement indisponibles — la certification reste valide mais les commissions ne seront pas prelevees.");
        }
        if (data.error?.includes("email") || data.error?.includes("resend") || data.error?.includes("Resend")) {
          // Non-blocking: certification succeeded but email failed
          setEmailSent(false);
          setResult(data);
          setStep("done");
          return;
      ...
      // Check if email was sent (API may return email_sent flag)
      setEmailSent(data.email_sent ?? null);
      setResult(data);
      setStep("done");
    } catch (err: any) {
  ...
  function resetForm() {
    setStep("intro");
    setPhoto1(null); setPhoto2(null); setPhoto3(null); setPhotoCreation(null);
    setTitle(""); setTechnique(""); setDimW(""); setDimH("");
    setYear(""); setDescription(""); setPrice("");
    setResult(null); setQualityScore(null); setSubmitError(null); setEmailSent(null);
  }

  // ── Nav buttons ────────────────────────────────────────
  function NavButtons({ back, next, nextDisabled, nextLabel }: {
    back?: Step; next?: () => void; nextDisabled?: boolean; nextLabel?: string;
  }) {
    ...
    <div className="max-w-lg mx-auto px-4 py-6 min-h-[calc(100vh-4rem)] pb-24">
      ...
      {step === "intro" && (
        <div className="animate-fade-in text-center pt-6">
          ...
          <div className="rounded-xl bg-yellow-500/5 border border-yellow-500/15 p-3 flex items-start gap-2.5 mb-6 text-left">
            <Mail className="size-4 text-yellow-400/70 shrink-0 mt-0.5" />
            <p className="text-[11px] text-white/35 leading-relaxed">
              Les notifications par email sont temporairement indisponibles. Votre certification sera bien enregistree — vous pouvez verifier son statut dans votre tableau de bord.
            </p>
          </div>
        ...
        <div className="animate-fade-in">
          ...
          {photo1 ? (
            ...
            <button onClick={() => capturePhoto(setPhoto1)}
              className="relative w-full aspect-[3/4] rounded-2xl border-2 border-dashed border-[#C9A84C]/30 flex flex-col items-center justify-center gap-4 bg-[#C9A84C]/5 active:bg-[#C9A84C]/10 mb-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={toggleListening}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  isListening ? "bg-red-500/20 text-red-400 animate-pulse" : "bg-white/5 text-white/50 active:bg-white/10"
                }`}>
      ...
      {step === "review" && (
        <div className="animate-fade-in">
          ...
          {submitError && (
            <div className="rounded-xl bg-red-500/10 border border-red-500/20 p-3 flex items-start gap-2.5 mb-4">
          ...
          {price && (
            <div className="rounded-xl bg-yellow-500/5 border border-yellow-500/15 p-3 flex items-start gap-2.5 mb-4">
              <AlertTriangle className="size-3.5 text-yellow-400/70 shrink-0 mt-0.5" />
              <p className="text-[11px] text-white/35">
                Paiements temporairement indisponibles — votre prix sera enregistre mais aucune commission ne sera prelevee pour l&apos;instant.
              </p>
            </div>
          )}
      ...
      {step === "done" && result && (
        <div className="animate-fade-in text-center pt-6">
          ...
          <p className="text-white/40 text-sm mb-4">Verification sous 24h</p>

          {/* Email notice if not sent */}
          {emailSent === false && (
            <div className="rounded-xl bg-yellow-500/5 border border-yellow-500/15 p-3 flex items-center gap-2.5 mb-6 text-left">
              <Mail className="size-4 text-yellow-400/70 shrink-0" />
              <p className="text-[11px] text-white/40">
                Email de confirmation non envoye (service temporairement indisponible). Votre certification est bien enregistree.
              </p>
            </div>
          )}
          ...
          <div className="space-y-3">
            {result.id && (
              <a href={`/art-core/oeuvre/${result.id}`}
