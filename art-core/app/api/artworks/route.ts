...
async function ensureMerchantForProUser(user: { id: string; email: string; role: string; full_name?: string; name?: string }) {
  ...
  // Profil minimal, à compléter par l'user ensuite
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Migration photos : legacy `photos` (TEXT JSON) + nouveau schéma
    ...
    // ── Hash blockchain déterministe pour la certification Pass-Core ─────
    // SHA-256 du contenu + photos + timestamp. Sert de preuve d'intégrité
    // (simulé tant que l'intégration on-chain Polygon/Base n'est pas en place).
    ...
    // INSERT artwork sans pass_core_id (FK vers pass_core — on le peuplera après)
    await query(
      ...
      [
        ...
        description || "", technique || "", dimensions || "", creation_date || "", category || "painting",
    ...
    // ── Création du Pass-Core (FK circulaire avec artworks) ───────────────
    // On crée le record pass_core APRÈS l'artwork, puis on met à jour
    ...
    try {
      ...
      const { data: passCore, error: pcErr } = await sb
        ...
        .insert({
          ...
          issuer_id: user.id, // plateform-less : l'émetteur est l'uploader pour l'instant
      ...
      if (pcErr) {
        ...
        // Mise à jour de l'artwork avec le pass_core_id nouvellement créé
      ...
      try {
        ...
        if (!merchant) {
          fichePolice = {
            ...
            message: "Fiche de police non générée — impossible de créer un profil merchant par défaut.",
          };
        } else {
