...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await queryOne("SELECT id FROM nova_accounts WHERE user_id = ?", [user.id]);
    ...
    await query(`INSERT INTO nova_accounts (
    ...
    // Update user
    await query("UPDATE users SET nova_account_id = ?, nova_account_type = ?, bank_partner_connected = 1 WHERE id = ?",
      [id, account_type, user.id]);

    // TODO: Credit bonus points via awardPoints function - needs async conversion
    ...
    if (account_type === "initiate") {
      await query("UPDATE users SET is_initie = 1 WHERE id = ?", [user.id]);
    }
    ...
    if (account_type === "ambassador") {
      ...
      await query("UPDATE nova_accounts SET kit_tracking_number = ?, kit_macro_shipped = 1 WHERE id = ?",
        [kitTracking, id]);
    }
    ...
    await query("INSERT INTO point_transactions (id, user_id, amount, type, reference_id, description) VALUES (?, ?, 80, 'nova_platform_fee', ?, ?)",
      [ptPlatform, "usr_admin_1", id, `Nova Bank — frais partenariat compte ${novaNumber}`]);
