...
CREATE TABLE public.scouted_artists (
  id                      UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- users
CREATE INDEX idx_users_role ON public.user_roles(role) WHERE is_active = TRUE;
...
CREATE INDEX idx_artworks_owner_id ON public.artworks(current_owner_id);
CREATE INDEX idx_artworks_status ON public.artworks(status);
CREATE INDEX idx_artworks_category ON public.artworks(category);
CREATE INDEX idx_artworks_published ON public.artworks(published_at DESC NULLS LAST) WHERE is_public = TRUE;
CREATE INDEX idx_artworks_for_sale ON public.artworks(price ASC) WHERE is_for_sale = TRUE AND is_public = TRUE;
CREATE INDEX idx_artworks_for_rent ON public.artworks(rental_price_per_day ASC) WHERE is_for_rent = TRUE AND is_public = TRUE;
CREATE INDEX idx_artworks_pass_core ON public.artworks(pass_core_id) WHERE pass_core_id IS NOT NULL;
CREATE INDEX idx_artworks_tags ON public.artworks USING GIN(tags);
CREATE INDEX idx_artworks_fts ON public.artworks
...
-- pass_core
CREATE INDEX idx_pass_core_artwork ON public.pass_core(artwork_id);
CREATE INDEX idx_pass_core_owner ON public.pass_core(current_owner_id);
CREATE INDEX idx_pass_core_hash ON public.pass_core(hash);
CREATE INDEX idx_pass_core_tx_hash ON public.pass_core(tx_hash);
CREATE INDEX idx_pass_core_status ON public.pass_core(status);

-- listings
CREATE INDEX idx_listings_artwork ON public.listings(artwork_id);
CREATE INDEX idx_listings_seller ON public.listings(seller_id);
CREATE INDEX idx_listings_status ON public.listings(status);
CREATE INDEX idx_listings_type ON public.listings(type);
CREATE INDEX idx_listings_featured ON public.listings(featured_until) WHERE featured = TRUE;
...
-- bets
CREATE INDEX idx_bets_auction ON public.bets(auction_id);
CREATE INDEX idx_bets_bidder ON public.bets(bidder_id);
CREATE INDEX idx_bets_status ON public.bets(status);
CREATE INDEX idx_bets_amount ON public.bets(auction_id, amount DESC);

-- transactions
CREATE INDEX idx_tx_buyer ON public.transactions(buyer_id);
CREATE INDEX idx_tx_seller ON public.transactions(seller_id);
CREATE INDEX idx_tx_artwork ON public.transactions(artwork_id);
CREATE INDEX idx_tx_status ON public.transactions(status);
CREATE INDEX idx_tx_type ON public.transactions(type);
CREATE INDEX idx_tx_stripe_pi ON public.transactions(stripe_payment_intent_id) WHERE stripe_payment_intent_id IS NOT NULL;
CREATE INDEX idx_tx_created ON public.transactions(created_at DESC);

-- royalties
CREATE INDEX idx_royalties_tx ON public.royalties(transaction_id);
CREATE INDEX idx_royalties_recipient ON public.royalties(recipient_id);
CREATE INDEX idx_royalties_unpaid ON public.royalties(created_at) WHERE paid_at IS NULL;

-- scouts
CREATE INDEX idx_scouts_user ON public.scouts(user_id);
CREATE INDEX idx_scouts_code ON public.scouts(affiliate_code);

-- affiliate_links
CREATE INDEX idx_affiliate_code ON public.affiliate_links(code);
CREATE INDEX idx_affiliate_scout ON public.affiliate_links(scout_id);
CREATE INDEX idx_affiliate_artwork ON public.affiliate_links(artwork_id) WHERE artwork_id IS NOT NULL;

-- subscriptions
CREATE INDEX idx_sub_user ON public.subscriptions(user_id);
CREATE INDEX idx_sub_status ON public.subscriptions(status);
CREATE INDEX idx_sub_stripe ON public.subscriptions(stripe_subscription_id) WHERE stripe_subscription_id IS NOT NULL;

-- anonymous_messages
CREATE INDEX idx_messages_recipient ON public.anonymous_messages(recipient_id);
CREATE INDEX idx_messages_sender ON public.anonymous_messages(sender_id) WHERE sender_id IS NOT NULL;
CREATE INDEX idx_messages_unread ON public.anonymous_messages(recipient_id) WHERE is_read = FALSE;
CREATE INDEX idx_messages_artwork ON public.anonymous_messages(artwork_id) WHERE artwork_id IS NOT NULL;

-- rentals
CREATE INDEX idx_rentals_artwork ON public.rentals(artwork_id);
