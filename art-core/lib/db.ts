...
import { buildFilterQs, convertPlaceholders, parseWhere, splitCommasNotInParens } from "./sql-translator";
...
async function restFetch(path: string, init?: RequestInit): Promise<Response> {
  ...
  return fetch(`${SUPA_URL}/rest/v1/${path}`, {
    ...
    // la DB (ex: page marketplace /art-core). Sans ça, Next.js cache la réponse
    // à l'infini en server-side et les nouvelles œuvres n'apparaissent pas tant
...
let _sbAdmin: SupabaseClient<Database> | null = null;
export function getDb(): SupabaseClient<Database> {
  ...
  _sbAdmin = createSupabaseAdminClient<Database>(url, key, {
