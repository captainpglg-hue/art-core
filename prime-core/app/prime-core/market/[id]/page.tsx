...
import { getMarketById, getBetsForMarket } from "@/lib/db";
...
export default async function MarketDetailPage({ params }: Props) {
  ...
  const totalPool = Number(market.total_yes_amount) + Number(market.total_no_amount);
  const yesPercent = totalPool > 0 ? (Number(market.total_yes_amount) / totalPool * 100).toFixed(1) : "50.0";
  const noPercent = totalPool > 0 ? (Number(market.total_no_amount) / totalPool * 100).toFixed(1) : "50.0";
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="rounded-2xl bg-[#141720] border border-white/8 p-6 mb-6">
        ...
        <div className="grid grid-cols-3 gap-4 mt-6 text-center">
          ...
          <div className="rounded-xl bg-white/3 p-3">
            <p className="text-lg font-bold text-white capitalize tabular-nums">{String(market.status).replace("_", " ")}</p>
      ...
      {market.status === "open" && (
        <BetForm marketId={id} question={market.question} oddsYes={Number(market.odds_yes)} oddsNo={Number(market.odds_no)} />
      )}
