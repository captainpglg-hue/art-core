...
const FROM = process.env.SMTP_FROM || process.env.RESEND_FROM_FALLBACK || "PRIME-CORE <onboarding@resend.dev>";
...
export async function POST(req: NextRequest) {
  ...
  const subject = String(body.subject || "Contact PRIME-CORE").trim().slice(0, 200);
  ...
  const html = `
    <h2>Nouveau message contact — PRIME-CORE</h2>
