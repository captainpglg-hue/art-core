...
function LoginPageInner() {
  ...
  const isPassCore = redirectTo.includes("pass-core");
  const isPrimeCore = redirectTo.includes("prime-core");
  const appName = isPassCore ? "PASS-CORE" : isPrimeCore ? "PRIME-CORE" : "ART-CORE";
    ...
    <div className="animate-fade-in">
      <div className="mb-8">
        ...
        <p className="text-white/50 text-sm mt-1">
          Accedez a votre espace {appName}
        </p>
      </div>
      ...
      <div className="mt-6 p-4 rounded-xl bg-white/5 border border-white/10 text-center">
        <p className="text-white/50 text-sm mb-3">Pas encore de compte ?</p>
        <Link
          href={`/auth/signup?redirectTo=${encodeURIComponent(redirectTo)}`}
          className="inline-flex py-3 px-8 rounded-xl bg-[#D4AF37] text-[#0a0a0a] font-semibold active:brightness-90"
        >
