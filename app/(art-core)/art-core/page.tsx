...
import { ClientArtworkGrid } from "@/components/art-core/ClientArtworkGrid";
import { ShieldCheck, Truck, RotateCcw, ArrowRight } from "lucide-react";

export const metadata: Metadata = {
  title: "Marketplace",
  description: "Decouvrez et achetez des oeuvres d'art uniques certifiees sur ART-CORE.",
};
...
async function ArtworkGrid({ searchParams }: { searchParams: Awaited<PageProps["searchParams"]> }) {
  ...
  try {
    ...
    // Fallback: return empty, client will see 0 results
    artworks = [];
  }

  let parsed = artworks.map((a: any) => ({
    ...a,
  ...
  if (pMin > 0) parsed = parsed.filter((a: any) => a.price >= pMin);
  if (pMax > 0) parsed = parsed.filter((a: any) => a.price <= pMax);
  ...
  return (
    <>
      ...
      <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-5 space-y-0">
        {sorted.map((artwork: any, i: number) => (
...
export default async function MarketplacePage({ searchParams }: PageProps) {
  ...
  return (
    <>
        ...
        <section className="bg-[#121212] border-b border-white/5">
          <div className="max-w-screen-xl mx-auto px-4 lg:px-8 py-12 md:py-16">
            <h1 className="font-playfair text-3xl md:text-5xl font-semibold text-white leading-tight mb-4 max-w-2xl">
              L&apos;art original, certifie et livre chez&nbsp;vous
            </h1>
            <p className="text-white/45 text-base md:text-lg mb-8 max-w-lg">
              Des milliers d&apos;oeuvres uniques d&apos;artistes emergents et reconnus
            </p>
            <Link href="#catalogue"
              ...
              Decouvrir les oeuvres <ArrowRight className="size-5" />
            </Link>

            {/* Reassurance */}
            <div className="flex flex-wrap gap-6 md:gap-10 mt-10">
              {[
                { icon: ShieldCheck, text: "Oeuvres certifiees originales" },
                { icon: Truck, text: "Livraison securisee sous 7 jours" },
                { icon: RotateCcw, text: "Satisfait ou rembourse 14 jours" },
              ].map((item) => {
      ...
      <div id="catalogue" className="sticky top-16 z-30 bg-[#0F0F0F]/95 backdrop-blur-xl border-b border-white/[0.06]">
