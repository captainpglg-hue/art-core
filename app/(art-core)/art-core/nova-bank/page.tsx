...
export default function NovaBankPage() {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 min-h-[calc(100vh-8rem)]">
      ...
      {/* Banner — Clip macro gratuit */}
      <div className="rounded-2xl bg-gradient-to-r from-blue-600/20 to-[#C9A84C]/20 border border-blue-500/20 p-5 mb-6">
        <div className="flex items-center gap-3 mb-3">
          <Package className="size-8 text-blue-400" />
          <div>
            <p className="text-white font-bold text-lg">Obtenez votre kit macro GRATUIT</p>
            <p className="text-white/40 text-xs">Ouvrez un compte Nova Bank — offre reservee Pass-Core</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 text-xs">
          {[
            { text: "Compte courant sans frais", icon: CreditCard },
            { text: "Carte Visa internationale gratuite", icon: CreditCard },
            { text: "IBAN francais immediat", icon: Globe },
            { text: "Kit macro Pass-Core offert (25€)", icon: Package },
            { text: "Virements SEPA illimites gratuits", icon: Coins },
            { text: "Pass-Core Premium 1 an offert (9.90€)", icon: Star },
          ].map((b) => (
            <div key={b.text} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/[0.03] border border-white/5">
              <b.icon className="size-3.5 text-[#C9A84C] shrink-0" />
        ...
        <div className="animate-fade-in space-y-3">
          ...
          <div className="rounded-xl bg-white/3 border border-white/5 p-4 mt-4">
            <p className="text-[11px] text-white/25 text-center leading-relaxed">
              Art-Core Solutions percoit une remuneration de Nova Bank pour chaque ouverture de compte.<br />
              Enregistre IOBSP aupres de l&apos;ORIAS. Offre soumise a conditions.
            </p>
          </div>
        </div>
      )}
        ...
        return (
          <div className="animate-fade-in">
            ...
            <div className={`rounded-2xl border ${acct.border} bg-gradient-to-b ${acct.color} p-5 mb-6 space-y-4`}>
              ...
              <div className="rounded-xl bg-green-500/5 border border-green-500/15 p-3 text-center">
                <p className="text-green-400 text-sm font-medium">Votre clip macro sera expedie sous 5 jours</p>
                <p className="text-white/30 text-[10px] mt-1">A l&apos;adresse renseignee lors de l&apos;inscription</p>
              </div>

              <p className="text-white/20 text-[10px] text-center">
                Nova Bank a verse 80 EUR a ART-CORE Solutions pour votre inscription partenaire.
                Cette commission nous permet de financer le developpement de la plateforme et vos avantages.
              </p>
            </div>
