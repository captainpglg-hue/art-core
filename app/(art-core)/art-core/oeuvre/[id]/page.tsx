...
export default async function ArtworkDetailPage({ params }: Props) {
  ...
  try {
    const sb = getDb();
    const { data: art } = await sb.from("artworks").select("views_count").eq("id", id).single();
    await sb.from("artworks").update({ views_count: (art?.views_count || 0) + 1 }).eq("id", id);
  } catch {}
        ...
        <div className="space-y-6 lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto no-scrollbar">
          ...
          <div className="flex flex-wrap gap-2">
            {isCertified && (
              <Badge className="text-[10px] bg-green-500/10 text-green-400 border-green-500/20 px-3 py-1">
                <ShieldCheck className="size-3 mr-1.5" />Œuvre originale certifiée
              </Badge>
            )}
            {isLocked && (
          ...
          <div className="rounded-xl border border-white/8 bg-[#1E1E1E] p-5 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <div className="group relative">
                <span className="text-white/20 hover:text-white/50 cursor-help text-xs">(?)</span>
                <div className="invisible group-hover:visible absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 rounded-xl bg-[#1a1a1a] border border-white/10 text-[11px] text-white/50 leading-relaxed shadow-xl z-50">
                  La jauge reflète l&apos;intérêt du marché pour cette œuvre. Plus elle monte, plus l&apos;œuvre prend de la valeur. Basée sur les vues, favoris et paris sur Prime-Core. À 100, le deal est verrouillé.
                </div>
              </div>
            </div>
            <GaugeBar
          ...
          {!isArtist && artwork.status !== "sold" && (
            <Link
              ...
              className="flex items-center justify-center gap-2 w-full py-3 rounded-xl border border-white/10 text-sm text-white/60 hover:text-white hover:bg-white/5 transition-all"
            >
              <MessageSquare className="size-4" />Contacter l&apos;artiste
            </Link>
          )}

          {/* Details */}
          <div className="space-y-1.5">
            <p className="text-xs uppercase tracking-widest text-white/25 mb-3">Details</p>
            {[
              ...
              artwork.creation_date && { icon: Calendar, label: "Creation", value: artwork.creation_date },
          ...
          {isCertified && (
            <div className="rounded-xl border border-gold/20 bg-gold/5 p-4 space-y-2">
              ...
              <div className="text-[11px] text-white/40 space-y-1">
                ...
                {artwork.certification_date && <p>Certifie le: <span className="text-white/60">{formatDate(artwork.certification_date)}</span></p>}
              </div>
            </div>
          )}
          ...
          {(markets || []).length > 0 && (
            <div>
              <p className="text-xs uppercase tracking-widest text-white/25 mb-3">Marches Predictifs (PRIME-CORE)</p>
              <div className="space-y-2">
                {(markets || []).map((m: any) => (
